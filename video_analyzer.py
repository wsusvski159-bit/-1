...
def _api_settings() -> tuple[str, str, str, str]:
    ...
    if not api_key:
        raise RuntimeError("还没有配置 NEWAPI_API_KEY。请在 Render 的 Environment 里填你这个 API 平台生成的 Key。")

    base_url = os.getenv("NEWAPI_BASE_URL", "https://api.qiyiguo.uk").strip().rstrip("/")
    primary = os.getenv("GEMINI_MODEL", "gemini-3.6-flash").strip()
    fallback = os.getenv("GEMINI_FALLBACK_MODEL", "gemini-2.5-pro").strip()
...
def analyze_video(video_path: Path, original_name: str) -> dict[str, Any]:
    ...
    if duration > max_duration:
        raise RuntimeError(f"视频约 {duration:.1f} 秒，超过当前 {max_duration:.0f} 秒上限。先剪到一分钟以内再给我看。")
